...
export const AboutSection: React.FC<AboutSectionProps> = ({ currentLang }) => {
  ...
  return (
    ...
    >
      {/* Top 3 Stats Row: Cards slide in from left/right with text fading */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
        {t.about.stats.map((stat, idx) => {
          // Determine slide direction: left for idx 0, center-bottom for idx 1, right for idx 2
          const xOffset = idx === 0 ? (isRtl ? 40 : -40) : idx === 2 ? (isRtl ? -40 : 40) : 0;
          ...
          return (
            <motion.div
              ...
              className={`rounded-2xl p-6 sm:p-7 flex flex-col items-center justify-center text-center gap-2 border ${
                stat.isAccent
                  ? 'bg-[var(--accent)] text-white border-transparent'
                  : 'surface-card bg-[var(--surface)] text-[var(--text-primary)]'
              }`}
            >
              <motion.span 
      ...
      {/* Main Bento: Left Mosaic (Sliding in from Left) + Right Bio (Sliding in from Right) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        {/* Left: 2-Column Balanced Mosaic - Slides from Left */}
        <motion.div 
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            {t.about.mosaic.slice(0, 2).map((item, idx) => (
              <div key={idx} className="surface-card rounded-2xl p-5 flex flex-col gap-3 justify-between">
                <div className="w-9 h-9 rounded-xl bg-[var(--accent-subtle)] text-[var(--accent)] flex items-center justify-center">
                ...
                <div>
                  ...
                  <motion.h4 
                    ...
                    className="font-header font-bold text-sm sm:text-base text-[var(--text-primary)] leading-snug mt-0.5"
                  >
                    {item.title}
                  </motion.h4>
                  <motion.p 
          ...
          <div className="flex flex-col gap-4">
            {t.about.mosaic.slice(2, 4).map((item, idx) => (
              <div key={idx} className="surface-card rounded-2xl p-5 flex flex-col gap-3 justify-between">
                ...
                <div>
                  <span className="font-mono text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-semibold">
                    {item.tag}
                  </span>
                  <motion.h4 
                    initial={{ opacity: 0 }}
        ...
        {/* Right: Rich Bio Card - Slides in from Right */}
        <motion.div 
          initial={{ opacity: 0, x: isRtl ? -50 : 50 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true, amount: 0.15 }}
          transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          className="lg:col-span-6 surface-card rounded-3xl p-7 sm:p-9 flex flex-col justify-between gap-6"
        >
          <div className="flex flex-col gap-4">
            <motion.div 
              initial={{ opacity: 0 }}
